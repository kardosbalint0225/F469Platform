...
/** @defgroup USBH_CORE_Private_Variables
  ...
  */
#if (USBH_USE_OS == 1U)
#define USBH_EVENT_QUEUE_LENGTH    MSGQUEUE_OBJECTS

static StackType_t   usbh_task_stack[USB_HOST_TASK_STACK_SIZE];
static StaticTask_t  usbh_task_tcb;

static StaticQueue_t usbh_event_queue_struct;
static uint8_t		 usbh_event_queue_storage[USBH_EVENT_QUEUE_LENGTH * sizeof(uint16_t)];
#endif
...
#if (USBH_USE_OS == 1U)
  phost->os_event = xQueueCreateStatic(USBH_EVENT_QUEUE_LENGTH,
		                               sizeof(uint16_t),
									   usbh_event_queue_storage,
									   &usbh_event_queue_struct);
...
USBH_StatusTypeDef USBH_ReEnumerate(USBH_HandleTypeDef *phost)
{
  ...
  (void)xQueueSend(phost->os_event, &phost->os_msg, 0U);
#endif
...
void USBH_LL_PortEnabled(USBH_HandleTypeDef *phost)
{
  ...
  (void)xQueueSendFromISR(phost->os_event, &phost->os_msg, &xHigherPriorityTaskWoken);
...
static void USBH_Process_OS(void *argument)
{
  for ( ;; )
  {
    if ( pdPASS == xQueueReceive( ((USBH_HandleTypeDef *)argument)->os_event, &((USBH_HandleTypeDef *)argument)->os_msg, portMAX_DELAY ) ) {
